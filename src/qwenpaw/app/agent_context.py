...
async def get_agent_for_request(
    ...
    config = load_config()
    target_agent_id = require_owned_agent_id(
        config,
        username,
        target_agent_id,
    )
...
def get_active_agent_id() -> str:
    ...
    try:
        ...
        if user_id:
            ensure_user_default_agent(user_id)
            resolved = resolve_requested_agent_id(config, user_id, None)
            if resolved:
                return resolved
