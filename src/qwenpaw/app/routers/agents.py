...
from ..user_space import (
    ...
    get_user_agent_order,
    get_user_default_agent_id,
    normalize_user_workspace_dir,
    remove_user_agent_reference,
    require_owned_agent_id,
    set_user_agent_order,
)
...
async def list_agents(request: Request) -> AgentListResponse:
    ...
    if username:
        ensure_user_default_agent(username)
    config = load_config()
    ordered_agent_ids = get_user_agent_order(config, username)
...
async def reorder_agents(
    ...
    """Persist the ordered list of visible agent IDs."""
    username = get_request_username(request)
    config = load_config()
    configured_ids = get_owned_agent_ids(config, username)
    ...
    if username:
        set_user_agent_order(username, list(reorder_request.agent_ids))
...
async def get_agent(
    ...
    try:
        config = load_config()
        actual_agent_id = require_owned_agent_id(
            config,
            get_request_username(request),
            agentId,
        )
        agent_config = load_agent_config(actual_agent_id)
        return agent_config
...
async def create_agent(
    ...
    When ``body.id`` is provided, it is used as the agent identifier
    ...
    if body.id:
        try:
            validate_agent_id(body.id, existing_ids)
    ...
    workspace_dir = normalize_user_workspace_dir(
        username,
        body.workspace_dir,
        new_id,
    )
    ...
    language = normalize_agent_language(
        body.language or config.agents.language or "en",
    )
    ...
    _initialize_agent_workspace(
        workspace_dir,
        skill_names=(
            body.skill_names if body.skill_names is not None else []
        ),
        language=language,
    )
    ...
    if username:
        set_user_agent_order(
            username,
            get_user_agent_order(load_config(), username),
        )

    logger.info(f"Created new agent: {new_id} (name={body.name})")
...
async def update_agent(
    ...
    existing_config = load_agent_config(actual_agent_id)
    ...
    existing_config.id = actual_agent_id
    save_agent_config(actual_agent_id, existing_config)
    schedule_agent_reload(request, actual_agent_id)
...
async def delete_agent(
    ...
    config = load_config()
    username = get_request_username(request)
    actual_agent_id = require_owned_agent_id(config, username, agentId)

    if not username and actual_agent_id == "default":
        raise HTTPException(
            status_code=400,
    ...
    await manager.stop_agent(actual_agent_id)

    del config.agents.profiles[actual_agent_id]
    config.agents.agent_order = _normalized_agent_order(config)
    save_config(config)
    if username:
        remove_user_agent_reference(username, actual_agent_id)

    return {"success": True, "agent_id": actual_agent_id}
...
async def toggle_agent_enabled(
    ...
    agent_ref = config.agents.profiles[actual_agent_id]
    manager = _get_multi_agent_manager(request)
    ...
    if enabled:
        try:
            await manager.get_agent(actual_agent_id)
            logger.info(f"Agent {actual_agent_id} started successfully")
        except Exception as e:
            logger.error(f"Failed to start agent {actual_agent_id}: {e}")
            raise HTTPException(
                status_code=500,
