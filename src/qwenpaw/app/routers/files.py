...
from qwenpaw.app.user_space import (
...
def _check_path(path: Path, request: Request | None = None) -> str | None:
    ...
    if username:
        ...
        config = load_config()
        for agent_id in get_owned_agent_ids(config, username):
            workspace_dir = Path(
                config.agents.profiles[agent_id].workspace_dir,
            ).expanduser().resolve()
            if resolved == workspace_dir or resolved.is_relative_to(workspace_dir):
                return None
        return "OUTSIDE_USER_WORKSPACE"
...
async def preview_file(
    ...
    path = path.resolve()
    reason = _check_path(path, request)
    if reason:
