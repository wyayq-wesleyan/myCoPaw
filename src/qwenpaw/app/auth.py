# -*- coding: utf-8 -*-
"""Authentication module: password hashing, JWT tokens, and middleware.

Supports a lightweight multi-user mode for internal/private deployments.
Each registered username authenticates independently and is later mapped to
its own isolated agent/workspace set by the application layer.
"""
...
def verify_token(token: str) -> Optional[str]:
    ...
    try:
        ...
        if not username or _get_user_record(username) is None:
            return None
        return username
...
def _load_auth_data() -> dict:
    ...
    if AUTH_FILE.is_file():
        try:
            ...
            data, migrated = _normalize_auth_data(data)
            if needs_rewrite:
                try:
                    _save_auth_data(data)
                ...
                try:
                    ...
                    logger.debug(
                        "Deferred auth.json multi-user migration: %s",
                        migrate_err,
                    )
            return data
...
def _normalize_auth_data(data: dict) -> tuple[dict, bool]:
    """Migrate auth data to the multi-user layout when needed."""
    changed = False
    ...
    for key, value in users.items():
        ...
        if not entry.get("role"):
            entry["role"] = "admin" if not normalized_users else "user"
            changed = True
        normalized_users[username] = entry

    if data.get("users") != normalized_users:
...
def _get_users(data: dict) -> dict[str, dict]:
    normalized, changed = _normalize_auth_data(data)
    if changed:
        _save_auth_data(normalized)
    return normalized.get("users", {})
...
def _get_user_record(username: str) -> dict | None:
    ...
    if data.get("_auth_load_error"):
        return None
    return _get_users(data).get(username)


def get_user_role(username: str) -> str | None:
    """Return the stored role for *username*."""
...
def register_user(
    ...
    if is_first_user:
        assign_legacy_agents_to_user(username)
...
def update_credentials(
    ...
    new_effective_username = current_username
    if new_username and new_username.strip():
        requested_username = new_username.strip()
        if requested_username != current_username and requested_username in users:
            raise ValueError("Username already exists")
        new_effective_username = requested_username
    ...
    if new_effective_username != current_username:
        users.pop(current_username, None)
    users[new_effective_username] = user
    data["users"] = users
    _save_auth_data(data)
    if new_effective_username != current_username:
        migrate_username(current_username, new_effective_username)
    ensure_user_default_agent(new_effective_username)
...
def authenticate(
    ...
    if data.get("_auth_load_error"):
        return None
    user = _get_users(data).get(username)
    if not user:
        return None
    stored_hash = user.get("password_hash", "")
    stored_salt = user.get("password_salt", "")
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        request.state.user_role = get_user_role(user)
        try:
